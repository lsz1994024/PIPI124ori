...
public class MassTool {
    ...
    public static final Map<Character, Double> n1514DiffMap = new HashMap<>();
    static {
    ...
    private final Pattern digestSitePatternForLinkSiteChecking1;
    private final Pattern digestSitePatternForLinkSiteChecking2;
    ...
    public MassTool(int missedCleavage, Map<Character, Double> fixModMap, String cleavageSite1, String protectionSite1, boolean cleavageFromCTerm1, String cleavageSite2, String protectionSite2, Boolean cleavageFromCTerm2, double ms2Tolerance, double oneMinusBinOffset, String labelling) {
        ...
        elementTable.put("Ac", elementTable.get("C") * 2 + elementTable.get("H") * 2 + elementTable.get("O"));

        this.missedCleavage = missedCleavage;
        this.ms2Tolerance = ms2Tolerance;
        inverse2Ms2Tolerance = 1 / (2 * ms2Tolerance);
        this.oneMinusBinOffset = oneMinusBinOffset; // the value is 1-(mz_bin_offset) in the parameter
        this.cleavageFromCTerm1 = cleavageFromCTerm1;
        this.cleavageFromCTerm2 = cleavageFromCTerm2;

        // masstable is the mass of residues
        ...
        massTable.put('#', massTable.get('I')); // I and L are treated as the same
        massTable.put('$', (massTable.get('Q') + massTable.get('K')) * 0.5); // Q and K are another pair of isobaric aa
        H2O = elementTable.get("H") * 2 + elementTable.get("O");

        // there are four letters that not used for aa, they are none
        massTable.put('B', 0d);
        massTable.put('J', 0d);
        massTable.put('X', 0d);
        massTable.put('Z', 0d);
        ...
        // trypsin cleaves next to arginine or lysine, but not before proline
        digestSitePattern1 = getDigestSitePattern(cleavageSite1, protectionSite1, cleavageFromCTerm1); // from CtoN or NtoC digest and skip protect site
        digestSitePatternForLinkSiteChecking1 = getDigestSitePatternForLinkSiteChecking(cleavageSite1, protectionSite1, cleavageFromCTerm1); // from NtoC or CtoN digest and skip protect site
    ...
    public MassTool(int missedCleavage, String cleavageSite1, String protectionSite1, boolean cleavageFromCTerm1, String cleavageSite2, String protectionSite2, Boolean cleavageFromCTerm2, double ms2Tolerance, double oneMinusBinOffset, String labelling) {
        ...
        massTable.put('n', fixModMap.get('n'));
        massTable.put('c', fixModMap.get('c'));
        massTable.put('#', massTable.get('I'));
        massTable.put('$', (massTable.get('Q') + massTable.get('K')) * 0.5);
        massTable.put('B', 0d);
        massTable.put('J', 0d);
        massTable.put('X', 0d);
        massTable.put('Z', 0d);
    ...
    public double calResidueMass(String sequence) {
        double totalMass = 0;
    ...
    public static AA[] seqToAAList(String sequence) {
        Matcher matcher = modAAPattern.matcher(sequence);
    ...
    public double[][] buildIonArray(String sequence, int maxCharge) {
        ...
        double bIonMass = massTable.get(aaArray[0].aa) + aaArray[0].ptmDeltaMass;
    ...
    public double buildVectorAndCalXCorr(double[][] ionMatrix, int precursorCharge, SparseBooleanVector xcorrPL) {
    ...
    static Pattern getDigestSitePattern(String cleavageSite, String protectionSite, boolean cleavageFromCTerm) {
        ...
        if (cleavageFromCTerm) {
            if (protectionSite.contentEquals("-")) {
                ...
                digestSitePattern = Pattern.compile("[" + cleavageSite + "](?![" + protectionSite + "])"); // ?! means don't need it
            }
        } else {
            if (protectionSite.contentEquals("-")) {
                digestSitePattern = Pattern.compile("[" + cleavageSite + "]");
            } else {
                digestSitePattern = Pattern.compile("(?<![" + protectionSite + "])" + "[" + cleavageSite + "]"); // ?<! means don't need it in reverse order
            }
        }
    ...
    private static Pattern getDigestSitePatternForLinkSiteChecking(String cleavageSite, String protectionSite, boolean cleavageFromCTerm) {
        ...
        if (cleavageFromCTerm) {
            if (protectionSite.contentEquals("-")) {
                ...
                digestSitePattern2 = Pattern.compile("[" + cleavageSite + "](?![" + protectionSite + "])$"); // $ finds it in the end of the string not in the beginning
            }
        } else {
            if (protectionSite.contentEquals("-")) {
                digestSitePattern2 = Pattern.compile("^[" + cleavageSite + "]");
